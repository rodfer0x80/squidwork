...
from squidwork.models.vits import MODELS as VITS_MODELS
from squidwork.models.vits import Y_LENGTH_ESTIMATE_SCALARS, HParams, Synthesizer, TextMapper, get_hparams_from_file, load_model
from squidwork.models.whisper import init_whisper, transcribe_waveform
...
class Listener:
    ...
    def listen(self):
        ...
        p = mp.Process(target=self.listener, args=(self.q, self.is_listening_event,))
        p.daemon = True
        p.start()
    ...
    def chat(self):
        ...
        with self.log_writer() as log:
            while True:
                ...
                prev_text = None
                while True:
                    for _ in range(self.listener.RATE // self.listener.CHUNK): total = np.concatenate([total, self.listener.q.get()])
                ...
                with Timing("tts: "):
                    ...
                    for i in sentences:
                        ...
                        for j in self.speaker.chunks(i.split(), self.speaker.max_sentence_length):
                            audio_data = self.speakertts(
                            " ".join(j), self.speaker.synth, self.speaker.hps, self.speaker.emotion_embedding,
                            self.speaker.vits_speaker_id, self.speaker.vits_model_to_use, self.speaker.vits_noise_scale,
                            self.speaker.vits_noise_scale_w, self.speaker.vits_length_scale,
                            self.speaker.vits_estimate_max_y_length, self.speaker.text_mapper, self.speaker.model_has_multiple_speakers
                            )
                            total = np.concatenate([total, audio_data])
                        self.speaker.out_q.put(total.tobytes())
                while self.speaker.out_counter.value < len(sentences): continue
